...
const names = useCookie('names_1', {
  default: () => [],
  maxAge: 300
})

const times = useCookie('times_1', {
  default: () => [],
  maxAge: 300
})

const Ytime = useCookie('usertime_1', {
  default: () => null,
  maxAge: 300
})
...
onMounted(async () => {
  ...
  if (names.value.length == 0 || times.value.length == 0) {
    ...
    const { data, status, error } = await useFetch('http://52.185.190.68:8000/api/rank-list/1', {
      method: 'GET',
      headers: {
